...
# Sub2API 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/changxiansub2api/main/deploy/install.sh | bash
#
...
get_latest_version() {
    ...
    LATEST_VERSION=$(github_api_curl -s --connect-timeout 10 --max-time 30 "https://api.github.com/repos/${GITHUB_REPO}/commits/main" 2>/dev/null | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
...
Documentation=https://github.com/changxian/sub2api
